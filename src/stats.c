...
static void refresh_stats(int s, short flags, void *arg)
{
	...
	add_safe_evtimer(pMainThread, &ev_stats, &period);
}
...
void stats_setup(void)
{
	...
	/* launch stats */
	pgbouncer_evtimer_set(pMainThread->eventBase, &ev_stats, refresh_stats, NULL);
	pgbouncer_evtimer_add(&ev_stats, &period);
}
