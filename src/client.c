...
bool set_pool(PgSocket *client, const char *dbname, const char *username)
{
	...
	if (!db) {
		...
		if (!db) {
			release_disconn_pgSocket(client, CL_DISCONN, true, "No such database: %s", dbname);
			return false;
		}
		else {
	...
	if (cf_auth_type == AUTH_ANY) {
		...
		if (db->forced_user == NULL) {
			...
			release_disconn_pgSocket(client, CL_DISCONN, true, "bouncer config error");
			return false;
		}
		...
		if (!user) {
			release_disconn_pgSocket(client, CL_DISCONN, true, "No such user: %s", username);
			return false;
		}
		client->auth_user = user;
	}

	/* pool user may be forced,config file user */
	if (db->forced_user)
		user = db->forced_user;

	client->pool = get_pool(db, user);//get db pool, when no pool, create a empty pool
	if (!client->pool) {
		release_disconn_pgSocket(client, CL_DISCONN, true, "no memory for pool");
		return false;
	}
	return true;
}
...
static bool decide_startup_pool(PgSocket *client, PktHdr *pkt)
{
	...
	while (1) {
		...
		if (strcmp(key, "database") == 0) {
			slog_debug(client, "got var: %s=%s", key, val);//f_game,pgbouncer
			dbname = val;
		} else if (strcmp(key, "user") == 0) {
			slog_debug(client, "got var: %s=%s", key, val);//harris
			username = val;
		} else if (varcache_set(&client->vars, key, val)) {//application_name=psql,client_encoding=UTF8
			slog_debug(client, "got var: %s=%s", key, val);
		} else if (strlist_contains(cf_ignore_startup_params, key)) {
			slog_debug(client, "ignoring startup parameter: %s=%s", key, val);
		} else {
			slog_warning(client, "unsupported startup parameter: %s=%s", key, val);
			release_disconn_pgSocket(client, CL_DISCONN, true, "Unsupported startup parameter: %s", key);
			return false;
		}
	}
	if (!username || !username[0]) {
		release_disconn_pgSocket(client, CL_DISCONN, true, "No username supplied");
		return false;
	}
...
static bool handle_client_startup(PgSocket *client, PktHdr *pkt)
{
	...
	if (incomplete_pkt(pkt)) {
		release_disconn_pgSocket(client, CL_DISCONN, true, "client sent partial pkt in startup phase");
		return false;
	}
	...
	case PKT_SSLREQ:
		...
		if (!sbuf_answer(&client->sbuf, "N", 1)) {
			release_disconn_pgSocket(client, CL_DISCONN, true, "failed to nak SSL");
			return false;
		}
		break;
	case PKT_STARTUP_V2:
		release_disconn_pgSocket(client, CL_DISCONN, true, "Old V2 protocol not supported");
		return false;
	case PKT_STARTUP:
		if (client->pool) {
			release_disconn_pgSocket(client, CL_DISCONN, true, "client re-sent startup pkt");
			return false;
		}
		...
		if (cf_auth_type <= AUTH_TRUST || client->own_user) {
			...
			if (!send_client_authreq(client)) {
				release_disconn_pgSocket(client, CL_DISCONN, false, "failed to send auth req");
				return false;
			}
		}
		break;
	case 'p':		/* PasswordMessage */
		/* too early */
		if (!client->auth_user) {
			release_disconn_pgSocket(client, CL_DISCONN, true, "client password pkt before startup packet");
			return false;
		}

		/* haven't requested it */
		if (cf_auth_type <= AUTH_TRUST) {
			release_disconn_pgSocket(client, CL_DISCONN, true, "unrequested passwd pkt");
			return false;
		}
		...
		if (ok && check_client_passwd(client, passwd)) {
			...
			release_disconn_pgSocket(client, CL_DISCONN, true, "Auth failed");
			return false;
		}
		break;
	case PKT_CANCEL:
		    ...
		    && mbuf_get_bytes(&pkt->data, BACKENDKEY_LEN, &key))
		{
			...
			release_disconn_pgSocket(client, CL_DISCONN, true, "bad cancel request");
		}
		return false;
	default:
		release_disconn_pgSocket(client, CL_DISCONN, false, "bad packet");
		return false;
	}
	sbuf_prepare_skip(sbuf, pkt->len);
...
static bool handle_client_work(PgSocket *client, PktHdr *pkt)
{
	...
	case 'Q':		/* Query */
		if (cf_disable_pqexec) {
			...
			release_disconn_pgSocket(client, CL_DISCONN, true, "PQexec disallowed");
			return false;
		}
	case 'F':		/* FunctionCall */
	...
	default:
		...
		release_disconn_pgSocket(client, CL_DISCONN, true, "unknown pkt");
		return false;
	case 'X': /* Terminate */
		release_disconn_pgSocket(client, CL_DISCONN, false, "client close request");
		return false;
	}
	return true;
}
...
bool client_proto(SBuf *sbuf, SBufEvent evtype, struct MBuf *data)
{
	...
	case SBUF_EV_RECV_FAILED:
		release_disconn_pgSocket(client, CL_DISCONN, false, "client unexpected eof");
		break;
	case SBUF_EV_SEND_FAILED:
		release_disconn_pgSocket(client->link, SV_DISCONN, false, "Server connection closed");
		break;
	case SBUF_EV_READ:
		...
		if (!get_header(data, &pkt)) {
			char hex[8*2 + 1];
			release_disconn_pgSocket(client, CL_DISCONN, true, "bad packet header: '%s'",
		...
		case CL_ACTIVE:
			...
			break;
		case CL_WAITING:
			fatal("why waiting client in client_proto(), tid: %u, current_thread_index: %d, sock: %d\n",
					(unsigned int)client->thread->thread->tid, client->thread->current_thread_index, client->sbuf.sock);
			break;
		default:
	...
	case SBUF_EV_FLUSH:
		...
		log_debug("sbuf_ev_flush, client is not interested in it");
		break;
