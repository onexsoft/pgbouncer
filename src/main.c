...
char *cf_server_reset_query; //DISCARD ALL
char *cf_server_check_query; //select 1
usec_t cf_server_check_delay;//30
...
int cf_threads_num ; // add by huih@20150722
usec_t cf_client_server_idle_timeout; //add by huih@20150722
usec_t cf_oldBouncer_socket_link_timeout;//add by huih@20150827
char* cf_ha_interface_name; //add by huih@20150831, virtual interface name
char* cf_ha_interface_vip; //add by huih@20150831, virtual ip

const char* pgbouncer_dbname = "pgbouncer";
PgThread *pMainThread;
/*
...
CF_ABS("max_client_conn", CF_INT, cf_max_client_conn, 0, "200"),
CF_ABS("default_pool_size", CF_INT, cf_default_pool_size, 0, "150"),
CF_ABS("min_pool_size", CF_INT, cf_min_pool_size, 0, "0"),
CF_ABS("reserve_pool_size", CF_INT, cf_res_pool_size, 0, "10"),
...
CF_ABS("server_reset_query", CF_STR, cf_server_reset_query, 0, "DISCARD ALL"),//DISCARD ALL
CF_ABS("server_check_query", CF_STR, cf_server_check_query, 0, ""),//select 1
CF_ABS("server_check_delay", CF_TIME_USEC, cf_server_check_delay, 0, "60000000"),
...
CF_ABS("client_login_timeout", CF_TIME_USEC, cf_client_login_timeout, 0, "60000000"),
...
CF_ABS("server_idle_timeout", CF_TIME_USEC, cf_server_idle_timeout, 0, "60000000"),
...
CF_ABS("stats_period", CF_INT, cf_stats_period, 0, "6000"),
...
CF_ABS("threads_num", CF_INT, cf_threads_num, 0, "1"),//add by huih@20150722
CF_ABS("client_server_idle_timeout", CF_TIME_USEC, cf_client_server_idle_timeout, 0, "1"),//add by huih@20150825
CF_ABS("oldBouncer_socket_link_timeout", CF_TIME_USEC, cf_oldBouncer_socket_link_timeout, 0, "60"),
CF_ABS("vip_address", CF_STR, cf_ha_interface_vip, 0, "172.30.12.15"),//192.168.0.1
CF_ABS("vip_interface_name", CF_STR, cf_ha_interface_name, 0, ""),//eth0:0
...
{
	int err = 0;
	event_assign(ev, base, fd, EV_SIGNAL | EV_PERSIST, cb, arg);
	err = event_add(ev, timeout);
...
{
	int err;
	struct event_base *base = pMainThread->eventBase;
#ifndef WIN32
	sigset_t set;
	...
	/* install handlers */
	err = pgbouncer_add_signal_event(base, &ev_sigusr1, SIGUSR1, handle_sigusr1, NULL, NULL);//user define
	if (err < 0)
		fatal_perror("signal_add");

	err = pgbouncer_add_signal_event(base, &ev_sigusr2, SIGUSR2, handle_sigusr2, NULL, NULL);//user define
	if (err < 0)
		fatal_perror("signal_add");

	err = pgbouncer_add_signal_event(base, &ev_sighup, SIGHUP, handle_sighup, NULL, NULL);//suspend
	if (err < 0)
		fatal_perror("signal_add");
#endif

	err = pgbouncer_add_signal_event(base, &ev_sigterm, SIGTERM, handle_sigterm, NULL, NULL);//kill
	if (err < 0)
		fatal_perror("signal_add");

	err = pgbouncer_add_signal_event(base, &ev_sigint, SIGINT, handle_sigint, NULL, NULL);//ctrl+c
	if (err < 0)
		fatal_perror("signal_add");
}
...
static void main_loop_once(void)
{
	...
	reset_time_cache();
	err = event_base_loop(pMainThread->eventBase, EVLOOP_ONCE);
	if (err < 0) {
		if (errno != EINTR)
			log_warning("event_loop failed: %s", strerror(errno));
	}
	run_timerEvent(pMainThread);
}

static void takeover_part1(void)
{
	/* use temporary libevent base */
	pMainThread = new_pgThread("main thread");
	if (pMainThread == NULL) {
		fatal("init globalConnect error");
	}
...
static void create_threads(void)
{
	//create vip thread
	if (init_vipThread()) {
		fatal("init vip thread error");
	}

	//create accept thread
	if (init_acceptThread()){
		fatal("init acceptThread error");
	}

	//create server pool thread
	if (init_serverPool()) { //add by huih@20150730
		fatal("serverPool_init error");
	}

	//create client thread
	if (init_clientThread()){//add by huih@20150723
		fatal("init_clientThread error");
	}
}
...
int main(int argc, char *argv[])
{
	...
	/* parse cmdline */
	while ((c = getopt_long(argc, argv, "qvhdVrRu:", long_options, &long_idx)) != -1) {
		switch (c) {
		case 'R':
			cf_reboot = 1;
			break;
	...
	init_objects(); //init user_tree,user_cache,db_cache,pool_cache
	load_config();
	main_config.loaded = true;
	init_caches();//server_cache,client_cache,io_cache
	...
	//add by huih@20150826
	create_threads();//in threads need uses pg_start_takeover
	...
	/*mult. thread stop*/
	destory_acceptThread(); //first
	destroy_clientThread(); //second, add by huih@20150723
	destory_serverPool();   //third, add by huih@20150730
	destory_vipThread(); //add by huih@20150831
