			...
			const char *timezone)
{
	...
	slog_error(admin, "sending socket list: fd=%d, len=%d",
...
//add by huih@20150828, when in multhreads, use the function to show client fds;
static bool show_fds_from_list_multhreads(PgSocket *admin, struct StatList *list)
{
	struct List *item;
	PgSocket *sk;
	bool res = true;
	...
	statlist_for_each(item, list) {
		...
		if (sk->pool != NULL && sk->pool->db->admin)
			continue;
		if (is_server_socket(sk) && NULL == sk->link)
			continue;
...
static bool admin_show_fds(PgSocket *admin, const char *arg)
{
	...
	statlist_for_each(item, &pool_list) {
		...
		res = res && show_fds_from_list(admin, &pool->old_active_server_list);
	...
	//add by huih@20150824, show client fds
	statlist_for_each(item, &clientThread_list) {
		pgCLThread = container_of(item, PgCLThread, head);
		res = res && show_fds_from_list(admin, &pgCLThread->activeList);
		res = res && show_fds_from_list(admin, &pgCLThread->oldActiveList);
		res = res && show_fds_from_list(admin, &pgCLThread->connectList);
		res = res && show_fds_from_list(admin, &pgCLThread->disConnectList);
		res = res && show_fds_from_list(admin, &pgCLThread->loginList);
		res = res && show_fds_from_list(admin, &pgCLThread->waitingList);
		if (!res)
			break;
	}
...
/*
 * Command: SHOW REBOOT_FDS
 *
 * If privileged connection, send also actual fds
 * when start use -R parameter, send active and waiting fds and
 * active server to restart application.
 */
static bool admin_show_reboot_fds(PgSocket *admin, const char *arg)
{
	struct List *item;
	PgPool *pool;
	bool res;
	...
	statlist_for_each(item, &pool_list) {
		...
		if (pool->db->admin)
			continue;
		res = res && show_fds_from_list_multhreads(admin, &pool->active_server_list);
		res = res && show_fds_from_list_multhreads(admin, &pool->old_active_server_list);
		if (!res)
			break;
	}
	...
	statlist_for_each(item, &clientThread_list) {
		pgCLThread = container_of(item, PgCLThread, head);
		res = res && show_fds_from_list_multhreads(admin, &pgCLThread->activeList);
		res = res && show_fds_from_list_multhreads(admin, &pgCLThread->waitingList);
		res = res && show_fds_from_list_multhreads(admin, &pgCLThread->oldActiveList);
		if (!res)
			break;
	}
...
#define SENDLIST(name, size) pktbuf_write_DataRow(buf, "si", (name), (size))
	...
	//SENDLIST("login_clients", statlist_count(&login_client_list));//delete by huih@20150730
...
static bool admin_show_clients(PgSocket *admin, const char *arg)
{
	...
	//add by huih@20150824
	statlist_for_each(item, &clientThread_list) {
		pgCLThread = container_of(item, PgCLThread, head);
		show_socket_list(buf, &pgCLThread->activeList, "active", false);
		show_socket_list(buf, &pgCLThread->connectList, "connect", false);
		show_socket_list(buf, &pgCLThread->disConnectList, "disconnect", false);
		show_socket_list(buf, &pgCLThread->justfreeList, "justfree", false);
		show_socket_list(buf, &pgCLThread->loginList, "login", false);
		show_socket_list(buf, &pgCLThread->waitingList, "waiting", false);
	}
...
static bool admin_show_sockets(PgSocket *admin, const char *arg)
{
	...
	//show client socket,add by huih@20150824
	statlist_for_each(item, &clientThread_list) {
		pgCLThread = container_of(item, PgCLThread, head);
		show_socket_list(buf, &pgCLThread->activeList, "cl_active", true);
		show_socket_list(buf, &pgCLThread->connectList, "cl_connect", true);
		show_socket_list(buf, &pgCLThread->disConnectList, "cl_disconnect", true);
		show_socket_list(buf, &pgCLThread->justfreeList, "cl_justfree", true);
		show_socket_list(buf, &pgCLThread->loginList, "cl_login", true);
		show_socket_list(buf, &pgCLThread->waitingList, "cl_waiting", true);
	}
	show_socket_list(buf, &cancel_client_list, "cl_cancel", true);
...
//		show_active_socket_list(buf, &pool->waiting_client_list, "cl_waiting");
	...
	//show_active_socket_list(buf, &login_client_list, "cl_login"); //delete by huih@20150730
...
static bool admin_show_pools(PgSocket *admin, const char *arg)
{
	...
	pktbuf_write_RowDescription(buf, "ssiiiii",
				    "database", "user",
				    "sv_active", "sv_idle",
				    "sv_used", "sv_tested",
				    "sv_login");
	statlist_for_each(item, &pool_list) {
		pool = container_of(item, PgPool, head);
		//waiter = first_socket_safe(&pool->mutex, &pool->waiting_client_list);
		pktbuf_write_DataRow(buf, "ssiiiii",
...
static bool admin_show_multhreads(PgSocket *admin, const char *arg)
{
	PktBuf *buf;
	struct List *item;
	PgCLThread *pgCLThread;
	...
	pktbuf_write_RowDescription(buf,"qsiiiiiiiiii", "tid", "name",
			"threadIndex", "thread_num", "current_thread_index",
			"connectSZ", "loginSZ", "waitingSZ",
			"activeSZ", "justfreeSZ", "disConnectSZ", "serverSZ");
	statlist_for_each(item, &clientThread_list) {
		pgCLThread = container_of(item, PgCLThread, head);
		pktbuf_write_DataRow(buf, "qsiiiiiiiiii", (unsigned int)pgCLThread->thread->tid,
				pgCLThread->thread->name, pgCLThread->thread->threadIndex,
				pgCLThread->thread_num, pgCLThread->current_thread_index,
...
static bool admin_cmd_shutdown(PgSocket *admin, const char *arg)
{
	 ...
	 */
	log_error("SHUTDOWN command issued");
	cf_shutdown = 2;

	shutdown_acceptThread();
	shutdown_clientThread();
	shutdown_serverPool();

	event_base_loopbreak(pMainThread->eventBase);
...
static bool admin_show_help(PgSocket *admin, const char *arg)
{
	...
	SEND_generic(res, admin, 'N',
		...
		"\tSHOW DNS_HOSTS|DNS_ZONES|MULTHREADS|REBOOT_FDS\n"
		"\tSET key = arg\n"
		"\tRELOAD\n"
		"\tPAUSE [<db>]\n"
		"\tRESUME [<db>]\n"
...
static struct cmd_lookup show_map [] = {
	...
	{"multhreads", admin_show_multhreads},
	{"REBOOT_FDS", admin_show_reboot_fds},
	{NULL, NULL}
};
...
static bool admin_parse_query(PgSocket *admin, const char *q)
{
	...
	current_query = NULL;
	if (!res)
		release_disconn_pgSocket(admin, CL_DISCONN, true, "failure");
	return res;
failed:
...
bool admin_handle_client(PgSocket *admin, PktHdr *pkt)
{
	...
	if (incomplete_pkt(pkt)) {
		release_disconn_pgSocket(admin, CL_DISCONN, true, "incomplete pkt");
		return false;
	}
	...
	case 'Q':
		if (!mbuf_get_string(&pkt->data, &q)) {
			release_disconn_pgSocket(admin, CL_DISCONN, true, "incomplete query");
			return false;
		}
		...
		return res;
	case 'X':
		release_disconn_pgSocket(admin, CL_DISCONN, false, "close req");
		break;
	default:
		admin_error(admin, "unsupported pkt type: %d", pkt_desc(pkt));
		release_disconn_pgSocket(admin, CL_DISCONN, true, "bad pkt");
		break;
	}
	return false;
}
...
bool admin_pre_login(PgSocket *client)
{
	...
	release_disconn_pgSocket(client, CL_DISCONN, true, "not allowed");
	return false;
}
...
void admin_pause_done(PgCLThread* pgCLThread)
{
	struct List *item, *tmp;
	PgSocket *admin;
	bool res;

	statlist_for_each_safe(item, &pgCLThread->activeList, tmp) {
		admin = container_of(item, PgSocket, head);

		if (!admin->pool->db->admin)
			continue;
		...
		if (!res)
			release_disconn_pgSocket(admin, CL_DISCONN, false, "dead admin");
		else
...
void admin_handle_cancel(PgSocket *admin)
{
	...
	if (!res)
		release_disconn_pgSocket(admin, CL_DISCONN, false, "readiness send failed");
}
