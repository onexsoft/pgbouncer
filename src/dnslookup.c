 ...
 *
 * Copyright (c) 2007-2010  Marko Kreen, Skype Technologies O脺
 *
...
struct DNSContext {
	...
	PgThread* thread; //add by huih@20150805
};
...
static bool impl_init(struct DNSContext *ctx)
{
	...
	event_base_set(ctx->thread->eventBase, &gctx->ev);
...
static bool impl_init(struct DNSContext *ctx)
{
	...
	ctx->edns = evdns_base_new(ctx->thread->eventBase, 1);
	if (!ctx->edns) {
...
static bool impl_init(struct DNSContext *ctx)
{
	...
	pgbouncer_event_set(ctx->thread->eventBase, &udns->ev_io,
			fd, EV_READ | EV_PERSIST, udns_io_cb, ctx);
	event_add(&udns->ev_io, NULL);

	/* timer setup */
	pgbouncer_evtimer_set(ctx->thread->eventBase, &udns->ev_timer, udns_timer_cb, ctx);
...
struct DNSContext *adns_create_context(PgThread *pgThread)
{
	struct DNSContext *ctx;
...
static void launch_zone_timer(struct DNSContext *ctx)
{
	...
	pgbouncer_evtimer_set(ctx->thread->eventBase, &ctx->ev_zone_timer, zone_timer, ctx);
	add_safe_evtimer(ctx->thread, &ctx->ev_zone_timer, &tv);
