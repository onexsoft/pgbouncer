...
void takeover_finish(void)
{
	...
	release_disconn_pgSocket(old_bouncer, SV_DISCONN, false, "disko over");
	old_bouncer = NULL;
}
...
//	bool res = false;
	...
	if (strcmp(task, "client") == 0) {
		add_clientOldSocket(get_oldPgSocket(fd, &addr, db, user, ckey, oldfd, linkfd,
				client_enc, std_string, datestyle, timezone, false));
	}
	else if (strcmp(task, "server") == 0) {
		add_serverOldSocket(get_oldPgSocket(fd, &addr, db, user, ckey, oldfd,
				linkfd, client_enc, std_string, datestyle, timezone, false));
	}
	else if (strcmp(task, "pooler") == 0) {
		add_acceptOldSocket(get_oldPgSocket(fd, &addr, NULL, NULL, 0, 0, 0,
...
static void next_command(PgSocket *bouncer, struct MBuf *pkt)
{
	...
	if (strcmp(cmd, "SUSPEND") == 0) {
		...
		SEND_generic(res, bouncer, 'Q', "s", "SHOW REBOOT_FDS;");
 ...
 */
bool takeover_login(PgSocket *bouncer)//bouncer is server
{
	bool res;
...
void takeover_init(void)
{
	...
	pool->thread = pMainThread;
	launch_new_connection(pool);//as a client, connect to old pgbouncer
}
