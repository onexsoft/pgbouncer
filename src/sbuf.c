...
bool sbuf_accept(SBuf *sbuf, int sock, bool is_unix)
{
	...
	sbuf->sock = sock;
	if (!tune_socket(sock, is_unix))//set socket attribute.
		goto failed;

	if (!cf_reboot) {
		res = sbuf_wait_for_data(sbuf);//set event,wait data
		if (!res)
			goto failed;
		/* socket should already have some data (linux only) */
		if (cf_tcp_defer_accept && !is_unix) {//tcp_defer_accept about tcp syn ack process
			sbuf_main_loop(sbuf, DO_RECV);//while accept-parse-send
			if (!sbuf->sock)
				return false;
		}
	}
...
bool sbuf_connect(SBuf *sbuf, const struct sockaddr *sa, int sa_len, int timeout_sec)
{
	 ...
	 * common stuff
	 */
	pgSocket = container_of(sbuf, PgSocket, sbuf);
	...
	log_debug("res: %d, errno: %d, sock: %d\n", res, errno, sock);
	if (res == 0) {
		...
		log_debug("add EV_WRITE, socket index: %d, socket state: %d, sock:%d",
				pgSocket->index, pgSocket->state, pgSocket->sbuf.sock);
		event_set(&sbuf->ev, sock, EV_WRITE, sbuf_connect_cb, sbuf);
		//add by huih@20150728
		event_base_set(pgSocket->thread->thread->eventBase, &sbuf->ev);
		res = event_add(&sbuf->ev, &timeout);
		if (res >= 0) {
			log_debug("add EV_WRITE event success, tid: %u", (unsigned int)pgSocket->thread->thread->tid);
...
bool sbuf_addReadEvent(SBuf *sbuf)
{
	if (sbuf->wait_type == W_NONE) {
		return sbuf_wait_for_data(sbuf);
	}
	return true;
}
...
bool sbuf_continue_with_callback(SBuf *sbuf, sbuf_libevent_cb user_cb)
{
	...
	AssertActive(sbuf);
	pPgSocket = container_of(sbuf, PgSocket, sbuf);
	log_debug("add EV_READ, socket index: %d, socket state: %d, sock:%d",
			pPgSocket->index, pPgSocket->state, pPgSocket->sbuf.sock);
	event_set(&sbuf->ev, sbuf->sock, EV_READ | EV_PERSIST,
		  user_cb, sbuf);
	event_base_set(pPgSocket->thread->thread->eventBase, &sbuf->ev);
...
bool sbuf_close(SBuf *sbuf)
{
	if (sbuf->wait_type) {
		...
		errno = 0;
		log_debug("event_del, sock: %d", sbuf->sock);
		if (event_del(&sbuf->ev) < 0) {
			log_error("event_del error, errmsg: %s", strerror(errno));
	...
	if (sbuf->io) {
		slab_free_safe(iobuf_cache, sbuf->io);
		sbuf->io = NULL;
	}
//	log_debug("finished sbuf_close");
	return true;
}
...
static bool sbuf_call_proto(SBuf *sbuf, int event)
{
	...
	if (event == SBUF_EV_PKT_CALLBACK)
		...
		iobuf_parse_all(io, &mbuf);//get all data from io, set this data to mbuf
	else
...
{
	int err;
	PgSocket *pgSocket = container_of(sbuf, PgSocket, sbuf);

	log_debug("sbuf.sock: %d, add EV_READ", sbuf->sock);
	pgbouncer_event_set(pgSocket->thread->thread->eventBase, &sbuf->ev,
			sbuf->sock, EV_READ | EV_PERSIST, sbuf_recv_cb, sbuf);
	err = event_add(&sbuf->ev, NULL);
	if (err < 0) {
		log_error("sbuf_wait_for_data: event_add: %s", strerror(errno));
		return false;
	}
	sbuf->wait_type = W_RECV;
	return true;
...
static void sbuf_send_cb(int sock, short flags, void *arg)
{
	...
	if (res) {
		...
		log_debug("read data, run sbuf_main_loop function now");
	...
	while (1) {
		...
		if (sbuf->pkt_action == ACT_SKIP || sbuf->pkt_action == ACT_CALL) {
			...
			if (iobuf_amount_pending(io) > 0) {
				res = sbuf_send_pending(sbuf);//send data, no use libevent
				if (!res)
					return res;
			}
		}
...
static bool sbuf_actual_recv(SBuf *sbuf, unsigned len)
{
	...
	got = iobuf_recv_limit(io, sbuf->sock, len);//recv data, no use libevent
	if (got == 0) {
		/* eof from socket */
		sbuf_call_proto(sbuf, SBUF_EV_RECV_FAILED);
		return false;
...
{
	if (sbuf->io == NULL) {
		sbuf->io = slab_alloc_safe(iobuf_cache);
		if (sbuf->io == NULL) {
			sbuf_call_proto(sbuf, SBUF_EV_RECV_FAILED);
			return false;
		}
...
static void sbuf_main_loop(SBuf *sbuf, bool skip_recv)
{
	...
	if (!allocate_iobuf(sbuf))//request memory for sbuf->io
		return;
...
try_more:
	...
	if (free > 0) {//
		...
		{
			free = sbuf->pkt_remain;//pkt_remain
		}
...
static void sbuf_connect_cb(int sock, short flags, void *arg)
{
	...
	if (flags & EV_WRITE) {
		if (!sbuf_after_connect_check(sbuf)) //checking connect happen error or not
			goto failed;
...
bool sbuf_is_pause(SBuf *sbuf)
{
	Assert(sbuf != NULL);
	return sbuf->wait_type == W_NONE;
}

bool sbuf_is_read(SBuf *sbuf)
{
	Assert(sbuf != NULL);
	return sbuf->wait_type == W_RECV;
}

void switch_thread(SBuf *sbuf)
{
	Assert(sbuf != NULL);
	if (!sbuf_is_pause(sbuf) && !sbuf_pause(sbuf)){
		fatal_perror("pause current event error");
		return;
	}
	if (!sbuf_wait_for_data(sbuf)) {
		fatal_perror("set read event error");
		return;
	}
}
