...
//	const PktBuf *pmsg = pool->welcome_msg;
	...
	if (msg->failed) {
		pktbuf_free(msg);
		release_disconn_pgSocket(client, CL_DISCONN, true, "failed to prepare welcome message");
		return false;
	}
	...
	pktbuf_free(msg);
	if (!res) {
		release_disconn_pgSocket(client, CL_DISCONN, true, "failed to send welcome message");
		return false;
	}
	return true;
}
