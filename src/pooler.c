...
static void pool_accept(int sock, short flags, void *arg)
{
	...
	if(!(flags & EV_READ)) {
		log_error("No EV_READ in pool_accept");
		return;
	}
	log_debug("pool accept sock: %d, flags: %d\n", sock, flags);
loop:
	/* get fd */
	fd = safe_accept(sock, &raddr.sa, &len);
	log_debug("safe_accept:sock: %d, fd: %d, errno: %d\n", sock, fd, errno);
	if (fd < 0) {
		if (errno == EAGAIN)
			return;
		...
		pgbouncer_evtimer_set(ls->multhread->eventBase, &ev_err, err_wait_func, NULL);
		add_safe_evtimer(ls->multhread, &ev_err, &err_timeout);
		suspend_pooler();
		return;
	}

	log_noise("new fd from accept=%d", fd);
	if (handle_client_use_clientThread(fd, is_unix) < 0)
	{
		fatal_perror("mulThread_handle_client error, fd=%d\n", fd);
		return ;
	}
	log_debug("P: get connection: %d\n", fd);
	/*
...
bool use_pooler_socket(int sock, bool is_unix)
{
	...
	if (!tune_socket(sock, is_unix)) {
		log_error("tune_socket error");
		return false;
	}

	ls = calloc(1, sizeof(*ls));
	ls->fd = sock;
	ls->active = false; //add by huih@20150826
	if (is_unix) {
		pga_set(&ls->addr, AF_UNIX, cf_listen_port);
	} else {
	...
	log_error("got pooler socket: %s", pga_str(&ls->addr, buf, sizeof(buf)));
	statlist_append(&sock_list, &ls->node);
	return true;
}
...
void resume_pooler(void)
{
	...
	statlist_for_each(el, &sock_list) {
		...
		if (ls->active)
			continue;
		log_error("sock: %d", ls->fd);
		pgbouncer_event_set(ls->multhread->eventBase, &ls->ev, ls->fd,
				EV_READ | EV_PERSIST, pool_accept, ls);
...
//add by huih@20150808
bool pooler_isActive(void)
{
	return pooler_active;
}

void set_acceptSocketEB(PgThread *pgThread)
{
	struct List *item;
	struct ListenSocket *ls;

	Assert(pgMulThread != NULL);
	statlist_for_each(item, &sock_list) {
		ls = container_of(item, struct ListenSocket, node);
		ls->multhread =  pgThread;
	}
}
