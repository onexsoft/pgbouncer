...
struct Slab {
	...
	pthread_mutex_t mutex; //add by huih@20150730 for multhread
};
...
void *slab_alloc_safe(struct Slab* slab)
{
	struct List *item = NULL;
	pthread_mutex_lock(&slab->mutex);
	item = slab_alloc(slab);
...
void slab_free_safe(struct Slab *slab, void *obj)
{
	pthread_mutex_lock(&slab->mutex);
	slab_free(slab, obj);
	pthread_mutex_unlock(&slab->mutex);
