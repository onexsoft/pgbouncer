...
#define LOG_TRACE_RUN_IN  do { \
		LOG_CONTEXT_DEF; \
		log_generic(LG_DEBUG, LOG_CONTEXT, "start tid:%u, File: %s, func: %s, line: %d\n", (unsigned int)pthread_self(), __FILE__, __func__, __LINE__);\
	} while(0);

#define LOG_TRACE_RUN_END do { \
		LOG_CONTEXT_DEF; \
		log_generic(LG_DEBUG, LOG_CONTEXT, "finished tid:%u, File: %s, func: %s, line: %d\n", (unsigned int)pthread_self(), __FILE__, __func__, __LINE__);\
	} while(0);

#define LOG_TRACE_RUN     log_generic(LG_DEBUG, LOG_CONTEXT, "tid:%u, File: %s, line: %d, func: %s\n", (unsigned int)pthread_self(), __FILE__, __LINE__, __func__);

/** Log warning message */
#define log_warning(fmt, args...) do { LOG_CONTEXT_DEF; \
		log_generic(LG_WARNING, LOG_CONTEXT, "tid:%u, File: %s, line: %d, func: %s "fmt, (unsigned int)pthread_self(), __FILE__, __LINE__, __func__,## args); \
	} while (0)

/** Log stats (liveness) message */
#define log_stats(fmt, args...) do { LOG_CONTEXT_DEF; \
		log_generic(LG_STATS, LOG_CONTEXT, "tid:%u, File: %s, line: %d, func: %s "fmt, (unsigned int)pthread_self(), __FILE__, __LINE__, __func__,## args); \
	} while (0)

/** Log info message */
#define log_info(fmt, args...) do { LOG_CONTEXT_DEF; \
		log_generic(LG_INFO, LOG_CONTEXT, "tid:%u, File: %s, line: %d, func: %s "fmt, (unsigned int)pthread_self(), __FILE__, __LINE__, __func__,## args); \
	} while (0)

/** Log debug message */
#define log_debug(fmt, args...) do { LOG_CONTEXT_DEF; \
		if (unlikely(cf_verbose > 0)) { \
			log_generic(LG_DEBUG, LOG_CONTEXT, "tid:%u, File: %s, line: %d, func: %s "fmt, (unsigned int)pthread_self(), __FILE__, __LINE__, __func__,## args); \
		} \
	} while (0)
...
#define log_noise(fmt, args...) do { LOG_CONTEXT_DEF; \
		if (unlikely(cf_verbose > 1)) { \
			log_generic(LG_NOISE, LOG_CONTEXT, "tid:%u, File: %s, line: %d, func: %s "fmt, (unsigned int)pthread_self(), __FILE__, __LINE__, __func__, ## args);\
		} \
	} while (0)

/** Log error message */
#define log_error(fmt, args...) do { LOG_CONTEXT_DEF; \
	log_generic(LG_ERROR, LOG_CONTEXT, "tid:%u, File: %s, line: %d, func: %s "fmt,(unsigned int)pthread_self(),  __FILE__, __LINE__, __func__, ## args);\
} while (0)

/** Log and die.  It also logs source location */
#define fatal(fmt, args...) do { LOG_CONTEXT_DEF; \
	log_fatal(__FILE__, __LINE__, __func__, false, LOG_CONTEXT, "tid:%u "fmt, (unsigned int)pthread_self(), ## args); \
	exit(1); } while (0)

/** Log strerror and die.  Error message also includes strerror(errno) */
#define fatal_perror(fmt, args...) do { LOG_CONTEXT_DEF; \
	log_fatal(__FILE__, __LINE__, __func__, true, LOG_CONTEXT, "tid:%u "fmt,(unsigned int)pthread_self(), ## args); \
	exit(1); } while (0)

/** Less verbose fatal() */
#define die(fmt, args...) do { LOG_CONTEXT_DEF; \
	log_generic(LG_FATAL, LOG_CONTEXT, "tid:%u, File: %s, line: %d, func: %s "fmt,(unsigned int)pthread_self(), __FILE__, __LINE__, __func__, ## args);\
	exit(1); } while (0)
